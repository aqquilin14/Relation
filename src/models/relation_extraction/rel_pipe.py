...
def score_relations(examples: Iterable[Example], threshold: float) -> Dict[str, Any]:
    """Score a batch of examples."""
    micro_prf = PRFScore()
    output_file = "./experiments/goldPred.txt"
    ...
    with open(output_file, 'w') as f:
        for example in examples:
            ...
            # code for printing gold and predicted data in a file
            f.write(f"\ngold: \n")
            for (k, v) in gold:
                f.write(f"({k}, {v})")
            f.write(f"\npredicted: \n")
    ...
    for example in examples:
        ...
        for key, pred_dict in pred.items():
            gold_labels = [k for (k, v) in gold.get((key[0]-1, key[1]-1), {}).items() if v == 1.0]
            for k, v in pred_dict.items():
                if v >= threshold:
